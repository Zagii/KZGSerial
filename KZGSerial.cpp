	...
	//	Serial.print(", nowy znak=");Serial.println(c);
		...
		switch(_mode)
		{
			...
			case KZGSerialMODE_CRC:
				...
				// komunikat <12;x;abc;xyz>BDE
				// dla _index=0 -> c=1; _index=1 ->c=2
				// > jest na 12 pozycji
				// BD jest na 13,14,15 pozycji, czyli
...
bool KZGSerial::checkCRC(char* crc)
{
	/*uint8_t c1=calcCRC();
	tmp[30];
	sprintf(tmp,"checkCRC R=%s, C=%02X",crc,c1);
	deb(tmp);
...
//
uint8_t KZGSerial::crc8(const char *addr, uint8_t len)
{
	char crc = 0;
